<template>
  <div id="app">
    ...
    <div class="visited-news">
      <div class="news-list">
        <p v-if="visitedItems.length > 0" class="recently-seen-header">
          Recently seen:
        </p>
        <ul>
          <item v-for="(item, index) in visitedItems" :key="index" :item="item" />
        </ul>
      </div>
    </div>
  </div>
...
export default {
  ...
  computed: {
    feeds: () => feeds,
    visitedItems () {
      const items = this.$store.state.recentlySeenItems
      return items.slice(Math.max(items.length - 3, 0))
    }
  }
}
</script>
